...
def parse_item(item):
    direction = item[0]
    lenght = item[1:]
    return direction, int(lenght)

def next_position(current, text):
    # current = [x,y]
    new_position = current[:]
    direction, lenght = parse_item(items)
...
def next_position2(current, text):
    # current = [x,y]
    #new_position = current[:]
    direction, lenght = parse_item(items)
    for _ in range(lenght):
...
if __name__ == "__main__":
    ...
    for items in first_wire:
        #first_points.append(next_position(first_points, items))
        next_position2(first_points, items)

    for items in second_wire:
    #    second_points.append(next_position(second_points[-1], items))
        next_position2(second_points, items)



    #for items in first_points:
    #    if items in second_points:
    ...
    for first in first_points:
        ...
        for second in second_points:
            second_count += 1
            if first == second:
                print('{} - wires={}'.format(first,first_count+second_count))
