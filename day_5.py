...
def parse_opcode(code):
    ...
    try:
        ...
        var2 = False
    return op_code, var1, var2


def operate(data, pointer):

    opcode = data[pointer]
    operator, var1, var2 = parse_opcode(opcode)

    if var1:
        value1 = int(data[pointer+1])
    else:
        try:
            value1 = int(data[int(data[pointer+1])])
        except:
            pass
    if var2:
        value2 = int(data[pointer+2])
    else:
        try:
            value2 = int(data[int(data[pointer+2])])
        except:
            pass

    if operator == 1:
        data[int(data[pointer + 3])] = str(value1 + value2)
        ...
        data[int(data[pointer + 1])] = user_input
...
if __name__ == "__main__":
    ...
    running = True
    while running:
        running, pointer = operate(the_data, pointer)
